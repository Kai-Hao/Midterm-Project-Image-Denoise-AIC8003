...
model_name = "DnCNN"  # "DnCNN" "Restormer"
model_path = "./checkpoints/best_DnCNN_model_with_pixelmodel.pth"

ImageDatasets = ImageReaderWithNoiseMap(Mode='test')
...
model_name = "DnCNNwithPixelModel"
...
for idx, (noisy, original) in enumerate(tqdm(ImageDataloader)):
    noisy = noisy.cuda()
    # B C H W to 1 H W C
    noisy_show = unnormalize(rearrange(noisy, 'b c h w -> b h w c').cpu().squeeze(0).numpy()) * 255
    noisy_show = cv2.cvtColor(noisy_show.astype(np.uint8), cv2.COLOR_RGB2BGR)
    cv2.imwrite(f"./temp/Noisy_{idx+1}.png", noisy_show)
    with torch.no_grad():
